    ...
    <div v-for="item in data" :key="item.key" class="select-filter-item">
      ...
      <el-scrollbar>
        <ul class="select-filter-list">
          <li v-for="option in item.options" :key="option.value" :class="{ active: selected[item.key] && selected[item.key].includes(option.value) }" @click="select(item, option)">
